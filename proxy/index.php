<?php
	class proxied {
		...
		function set_options () {
			...
			if (ereg('url=', $_SERVER['HTTP_REFERER'])) {
				preg_match('#url=([^&]+)#', $_SERVER['HTTP_REFERER'], $referer);
				$referer = base64_decode($referer[1]);
				$options[CURLOPT_REFERER] = $referer;
			}
			if (!eregi('google\.bg', $this->url)) {
					...
					switch ($tag_name) {
						case 'form':
							if (strtolower($attributes['method']) == 'get' || !$attributes['method']) {
								...
								$extra_html = "<input type=\"hidden\" name=\"url\" value=\"$url\" /><input type=\"hidden\" name=\"__proxy_action\" value=\"redirect_get\" />";
								$attributes['action'] = './';
						...
						case 'head':
							if ($_GET['__proxy_form'] != '0') {
								...
								$extra_html .= "var url = '{$this->url_first}';\n";
		...
		function encode_url ($string, $raw = false, $plain = false) {
			...
			return $raw ? $string : (!$plain ? './?url=' : '') . base64_encode($string);
		}
		
		function handle_actions () {
			if ($_POST['__proxy_action'] == 'redirect_get') {
				$url = base64_decode($_POST['url']);
				unset($_POST['__proxy_action'], $_POST['url']);
				$get = '';
				foreach ($_POST as $key => $value) {
				...
				header('Location: ./?url=' . base64_encode($_POST['url']));
				exit;
			}
		}
		
	}
	$url = @parse_url($_GET['url']) && strlen($_GET['url']) ? base64_decode($_GET['url']) : false;
