...
// $json_string = $this->file_get_contents_curl('http://opensharecount.com/count.json?url=' . $this->url);
$json_string = $this->file_get_contents_curl('https://opensharecount.com/count.json?url=' . $this->url);
...
$json_string = $this->file_get_contents_curl('https://www.stumbleupon.com/services/1.01/badge.getinfo?url='.$this->url);
$json = json_decode($json_string, true);
return isset($json['result']['views'])?intval($json['result']['views']):0;
}
function get_delicious() {
$json_string = $this->file_get_contents_curl('http://feeds.del.icio.us/v2/json/urlinfo/data?url='.$this->url);
$json = json_decode($json_string, true);
...
$return_data = $this->file_get_contents_curl('https://api.pinterest.com/v1/urls/count.json?url='.$this->url);
